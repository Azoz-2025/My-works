...
# دالة للتأكد من أن المسار هو مستودع Git صالح
...
def commit_changes():
    try:
        # التأكد أولاً إذا كان هناك تغييرات غير مضافة
        status = subprocess.run(["git", "status", "--porcelain"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        if status.stdout.decode().strip() == "":  # إذا لم تكن هناك تغييرات
            messagebox.showwarning("تحذير", "لا توجد تغييرات لتوثيقها.")
            return

        # إضافة كافة الملفات غير المتعقبة أو المعدلة
        subprocess.run(["git", "add", "."], check=True)
        
        # التأكد من أن الملف تم إضافته
        subprocess.run(["git", "add", "../Repo.py"], check=True)  # إضافة الملف المحدد إذا كان غير مضاف
